...
exports.encode = function (node, enc) {
  var buf = messages.Node.encode({
    key: node.key,
    value: enc ? enc.encode(node.value) : node.value,
    heads: node.heads,
    trie: compressTrie(node.trie)
  })

  return buf
}

exports.decode = function (buf, enc) {
  var node = messages.Node.decode(buf)
  node.trie = decompressTrie(node.trie)
  node.value = enc ? enc.decode(node.value) : node.value
  return node
}
...
function decompressTrie (buf) { // return JSON.parse(buf)
  var trie = []
  var ptr = 0
  var delta = 0
