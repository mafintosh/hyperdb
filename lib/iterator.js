...
Iterator.prototype._open = function (cb) {
  ...
  this._db.get(this._prefix, opts, onheads)
...
function next (worker, cb) {
  ...
  // and we should stop, or it's because the latest value hasn't resolved
  // yet. If that's the case wait for it to do so ...
  ...
  for (var i = top.i; i < end; i++) {
    ...
    // TODO: add option that disables this
    var sortEnd = iterator._start === i ? 4 : 5
