...
function put (db, heads, key, value, cb) {
  var req = new PutRequest(db, key, value)
  req.start(heads, cb)
}

function PutRequest (db, key, value) {
  this.key = key
  this.value = value
...
PutRequest.prototype._finalize = function () {
  ...
  var writable = this._db._feeds[this._db._id]
  // TODO: this is *wrong*, clock should be generated from the heads
  // ie clock[feed] = max(heads.clocks[feed])
  var clock = []

  if (!writable) writable = this._db._feeds[this._db._id] = []

  for (var i = 0; i < this._db._feeds.length; i++) {
    clock[i] = (this._db._feeds[i] || []).length
    if (i === this._db._id) clock[i]++
  }
...
PutRequest.prototype._updateHead = function (worker, feed, seq) {
  ...
  worker.pending++
  this._db._getPointer(feed, seq, function (err, node) {
...
PutRequest.prototype._fork = function (worker, ptrs) {
  ...
  worker.pending++
  this._db._getAllPointers(ptrs, function (err, nodes) {
