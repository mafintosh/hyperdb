...
function generateFailingTest (name, dbCount, writeOps, cb) {
  console.log('in generateFailingTest, arguments:', arguments)
  writeOps.push((err => {
    t.error(err)
    t.end()
  }).toString())
...
function fuzzRunner (opts, cb) {
  ...
  makeDatabases(opts, function (dbs, replicateByIndex) {
    ...
    function doRun (err) {
      console.log('in doRun, arguments:', arguments)
      if (err) {
        var failingBatches = ops.slice(0, finished + 1)
        generateFailingTest(`failing-test-${testNum}`, opts.writers, failingBatches, err => {
          return cb(err)
        })
      }
